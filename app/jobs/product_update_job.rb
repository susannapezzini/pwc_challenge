...
class ProductUpdateJob < ApplicationJob
  ...
  def perform(bank_id,bp_bank_id, bp_pdf_url, prod_type, cloud_merged_url, request_id)

    def return_dic_with_pages(prod_type, request_id)
    ...
    @payload = {
      "#{@bank.id}": {
        ...
        "products": return_dic_with_pages(prod_type, request_id)
      }
    }
    # @payload = {
    #   "#{@bank.id}": {
    ...
    @result = HTTParty.post('https://bank-price-api.herokuapp.com/get_stats', 
      ...
      if @result['status'] == 'ok'
        ...
        @request.update(raw_data: @data.values)
        # byebug
